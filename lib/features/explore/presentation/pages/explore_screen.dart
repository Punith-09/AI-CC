...
class _ExploreScreenState extends State<ExploreScreen> {
  ...
  List<TalentModel> _getFilteredTalents(List<TalentModel> talents) {
    switch (selectedCategory) {
      case 1:
        return talents;
      case 2:
        return talents.where((e) => e.role.toLowerCase() == "actor").toList();
      case 3:
        return talents.where((e) => e.role.toLowerCase() == "model").toList();
      case 4:
        return talents.where((e) => e.role.toLowerCase() == "singer").toList();
      default:
        return talents;
    }
  }
                    ...
                    Expanded(
                      child: Consumer<ExploreProvider>(
                        builder: (context, provider, child) {
                          ...
                          // If no data is available from API, we can fallback to empty
                          final talents = provider.talents;
                          if (talents.isEmpty) {
                            return const Center(
                              child: Text('No talents found', style: TextStyle(color: Colors.white)),
                            );
                          }

                          return TalentGrid(
                            talents: _getFilteredTalents(talents),
                          );
                        },
                      ),
                    ),
